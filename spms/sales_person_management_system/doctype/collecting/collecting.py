...
class Collecting(WebsiteGenerator):
    def on_submit(self) -> None:
        ...
        # if target type is 'Fixed Type'
        if collect_goal_doc.target_type == "Fixed Target":
            collect_goal_doc.total_collected += frappe.utils.flt(
                self.amount_other_currency)

        # if target type is 'According to Customer indebtedness' OR 'Debt + Additional Target'
        else:
            ...
            if customer_not_found:
                collect_goal_doc.additional_collected = collect_goal_doc.additional_collected + \
                    self.amount_other_currency
        ...
        if len(collect_goal_doc.commissions_range) != 0:
            target_percentage = (
                collect_goal_doc.total_collected / collect_goal_doc.total_targets) * 100
            for i in collect_goal_doc.commissions_range:
                if target_percentage >= i.from_ and target_percentage <= i.to_:
                    total_incentives = (i.commission/100) * \
                        collect_goal_doc.total_collected
                    break
    ...
    def on_cancel(self) -> None:
        ...
        total_incentives = 0
        # Calculating the incentives for the sales person based on the total collected amount.
        if len(collect_goal_doc.commissions_range) != 0:
            target_percentage = (
                collect_goal_doc.total_collected / collect_goal_doc.total_targets) * 100
            for i in collect_goal_doc.commissions_range:
